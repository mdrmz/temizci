    ...
    <div class="app-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                ...
                <div class="page-subtitle">KaydettiÄŸiniz ilanlarÄ± buradan takip edin</div>

                <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;">
                    <a href="favorites.php" class="btn btn-sm <?= $filterStatus === '' ? 'btn-primary' : 'btn-outline' ?>">TÃ¼m</a>
                    <a href="favorites.php?status=open" class="btn btn-sm <?= $filterStatus === 'open' ? 'btn-primary' : 'btn-outline' ?>">AÃ§Ä±k</a>
                ...
                <?php if (empty($favorites)): ?>
                    <div class="card mt-4">
                        <div class="empty-state" style="padding:60px 20px;">
                            ...
                            <p style="color:var(--text-muted);margin-bottom:20px;">Ä°lanlarÄ± incelerken kalp ikonuna tÄ±klayarak favorilere ekleyebilirsiniz.</p>
                            <a href="listings/browse" class="btn btn-primary">Ä°lanlarÄ± KeÅŸfet</a>
                        </div>
                    </div>
                <?php else: ?>
                            ...
                            <div class="card listing-card" id="fav-card-<?= $fav['id'] ?>">
                                <?php if (!empty($fav['home_photo'])): ?>
                                    <div class="card-img-placeholder" style="background: url('<?= APP_URL ?>/uploads/homes/<?= $fav['home_photo'] ?>') center/cover no-repeat; position: relative; border-radius: var(--radius-lg) var(--radius-lg) 0 0;">
                                        ...
                                        <button class="fav-remove-btn" onclick="removeFavorite(<?= $fav['id'] ?>)"
                                            style="position:absolute;top:12px;right:12px;background:rgba(0,0,0,0.55);border:none;border-radius:50%;width:36px;height:36px;cursor:pointer;font-size:1.05rem;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);transition:all 0.2s;color:#ef4444;"
                                            title="Favoriden KaldÄ±r">&#10084;</button>
                                    </div>
                                ...
                                <?php else: ?>
                                    <div class="card-img-placeholder" style="position:relative;">
                                        <?= $fav['cat_icon'] ?>
                                        <button class="fav-remove-btn" onclick="removeFavorite(<?= $fav['id'] ?>)"
                                            style="position:absolute;top:12px;right:12px;background:rgba(255,255,255,0.9);border:none;border-radius:50%;width:36px;height:36px;cursor:pointer;font-size:1.05rem;display:flex;align-items:center;justify-content:center;transition:all 0.2s;color:#ef4444;"
                                            title="Favoriden KaldÄ±r">&#10084;</button>
                                    </div>
                                    <div class="card-content">
                                ...
                                <?php endif; ?>
                                    ...
                                    <div class="listing-footer">
                                        ...
                                        <?php else: ?>
                                            <span style="color:var(--text-muted);font-size:0.85rem;">BÃ¼tÃ§e belirsiz</span>
                                        <?php endif; ?>
        ...
        function removeFavorite(listingId) {
            ...
            .then(data => {
                if (data.status === 'success' && data.action === 'removed') {
                    ...
                    alert(data.message || 'Favori kaldÄ±rÄ±lÄ±rken bir hata oluÅŸtu.');
                }
            })
            .catch(() => {
