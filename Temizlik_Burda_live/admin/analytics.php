...
<head>
    ...
    <title>Admin Analitik — <?= APP_NAME ?></title>
...
<body>
    <div class="app-layout">
        ...
        <div class="main-content">
            <?php $headerTitle = 'Platform Analitik'; include __DIR__ . '/includes/header.php'; ?>
            <div class="page-content">
                ...
                <div class="analytics-grid mt-4">
                    ...
                    <div class="ana-card">
                        ...
                        <div class="ana-label">İlan Kapanış Oranı</div>
                        <div class="ana-change" style="color:var(--text-muted);">Destek çözüm oranı: <?= number_format($supportCloseRate, 1) ?>%</div>
                    </div>
                    <div class="ana-card">
                        <div class="ana-val" style="color:#8b5cf6;"><?= number_format($counterAcceptRate, 1) ?>%</div>
                        <div class="ana-label">Pazarlık Kabul Oranı</div>
                        <div class="ana-change" style="color:var(--text-muted);"><?= $totalCounterAccepted ?> / <?= $totalCounterOffers ?> karşı teklif kabul</div>
                    </div>
                </div>
                ...
                <div class="card mt-4">
                    <div class="card-header"><div class="card-title">En Yoğun Bölgeler</div></div>
                    <div class="card-body" style="padding:0;">
                        <?php if (empty($topDistricts)): ?>
                            <div style="padding:18px;color:var(--text-muted);">Veri bulunamadı.</div>
                        <?php else: ?>
                ...
                <div class="card mt-4">
                    <div class="card-header"><div class="card-title">Son Pazarlık Hareketleri</div></div>
                    <div class="card-body" style="padding:0;">
                        <?php if (empty($recentNegotiations)): ?>
                            <div style="padding:18px;color:var(--text-muted);">Henüz pazarlık hareketi yok.</div>
                        <?php else: ?>
                            <?php foreach ($recentNegotiations as $neg): ?>
                                <?php
                                    $negLabel = match ($neg['event_type']) {
                                        'offer_sent' => 'Teklif gönderildi',
                                        'counter_sent' => 'Karşı teklif gönderildi',
                                        'counter_accepted' => 'Karşı teklif kabul edildi',
                                        'counter_rejected' => 'Karşı teklif reddedildi',
