    ...
    <div class="app-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-content" style="max-width: 860px; margin: 0 auto;">
                ...
                <div class="mb-4">
                    <a href="tickets.php" class="btn btn-outline btn-sm">Taleplere Geri Dön</a>
                </div>
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <span class="badge <?= ($ticket['priority'] ?? 'normal') === 'high' ? 'badge-closed' : ((($ticket['priority'] ?? 'normal') === 'low') ? 'badge-open' : 'badge-progress') ?>" style="margin-left:6px;">
                            <?= ($ticket['priority'] ?? 'normal') === 'high' ? 'Yüksek' : ((($ticket['priority'] ?? 'normal') === 'low') ? 'Düşük' : 'Normal') ?>
                        </span>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Yanıt Gönder</div>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            <?= csrfField() ?>
                            <div class="form-group">
                                ...
                                <textarea name="message" class="form-control" rows="5" required placeholder="Yanıtınızı yazın..."></textarea>
                            </div>
                            <div class="grid-2" style="align-items:end;">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="priority" class="form-control">
                                        <option value="high" <?= ($ticket['priority'] ?? 'normal') === 'high' ? 'selected' : '' ?>>Yüksek</option>
                                        <option value="normal" <?= ($ticket['priority'] ?? 'normal') === 'normal' ? 'selected' : '' ?>>Normal</option>
                                        <option value="low" <?= ($ticket['priority'] ?? 'normal') === 'low' ? 'selected' : '' ?>>Düşük</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Admin Notu (İç not)</label>
                                <textarea name="admin_note" class="form-control" rows="3" placeholder="Kısa takip notu..."><?= e($ticket['admin_note'] ?? '') ?></textarea>
                            </div>
                            <button type="submit" class="btn btn-primary">Yanıtla ve Güncelle</button>
                        </form>
                    </div>
                </div>
            </div>
