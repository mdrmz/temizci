...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_ticket') {
    if (verifyCsrf()) {
        ...
        if ($subject && $message) {
            ...
            setFlash('success', 'Destek talebiniz oluÅŸturuldu. En kÄ±sa sÃ¼rede yanÄ±tlanacaktÄ±r.');
            redirect('destek');
        }
    }
}
    ...
    <div class="app-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                ...
                <div class="page-subtitle">SorunlarÄ±nÄ±zÄ± buradan bildirin, ekibimiz size yardÄ±mcÄ± olsun.</div>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <?php if(empty($tickets)): ?>
                                        <tr><td colspan="6" style="text-align:center; padding:30px; color:var(--text-muted);">HenÃ¼z bir destek talebiniz yok.</td></tr>
                                    <?php endif; ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?= $t['status'] === 'open' ? 'badge-open' : ($t['status'] == 'in_progress' ? 'badge-progress' : 'badge-closed') ?>">
                                                <?= $t['status'] === 'open' ? 'AÃ§Ä±k' : ($t['status'] === 'in_progress' ? 'Ä°ÅŸlemde' : 'KapandÄ±') ?>
                                            </span>
                                        </td>
                                        <td>
                                            <span class="badge <?= $t['priority'] === 'high' ? 'badge-closed' : ($t['priority'] === 'low' ? 'badge-open' : 'badge-progress') ?>">
                                                <?= $t['priority'] === 'high' ? 'YÃ¼ksek' : ($t['priority'] === 'low' ? 'DÃ¼ÅŸÃ¼k' : 'Normal') ?>
                                            </span>
                                        </td>
                                        <td style="font-size:0.8rem;"><?= date('d.m.Y H:i', strtotime($t['created_at'])) ?></td>
                                        <td><a href="ticket_detail?id=<?= $t['id'] ?>" class="btn btn-ghost btn-sm">GÃ¶rÃ¼ntÃ¼le</a></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="card-body">
                            <form method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="subject" class="form-control" placeholder="Ã–rn: Ã–deme sorunu" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">MesajÄ±nÄ±z</label>
                                    <textarea name="message" class="form-control" rows="5" placeholder="LÃ¼tfen detaylÄ±ca aÃ§Ä±klayÄ±n..." required></textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <button type="submit" class="btn btn-primary btn-block">Talebi GÃ¶nder</button>
                            </form>
                        </div>
                    </div>
