...
// Kullanici zaten teklif verdi mi?
$hasOffer = false;
    ...
    if (!verifyCsrf()) {
        ...
        if ($offerId <= 0 || $counterPrice <= 0) {
            ...
            if (!$row) {
                ...
                createNotification((int) $row['user_id'], 'counter_offer', 'İlan sahibi teklifinize karşı teklif sundu.', '/listings/detail.php?id=' . $id);
                setFlash('success', 'Karşı teklif gönderildi.');
...
if (
    ...
    if (!verifyCsrf()) {
        ...
        if (!preg_match('/^\d{6}$/', $completionCode)) {
            $errors[] = '6 haneli güvenli onay kodu giriniz.';
        }
        $newProofPhoto = null;
        if (isset($_FILES['proof_photo']) && (int) ($_FILES['proof_photo']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
            ...
            if ($uploaded === false) {
                ...
                $errors[] = 'Kanıt fotoğrafı yüklenemedi.' . ($uploadReason !== '' ? ' ' . $uploadReason : '');
            } else {
        ...
        if (isset($_FILES['proof_photos']) && is_array($_FILES['proof_photos']['name'] ?? null)) {
            ...
            for ($i = 0; $i < $totalFiles; $i++) {
                ...
                if ($u === false) {
                    $uploadReason = getUploadError();
                    $errors[] = 'Galeri fotoğrafları yüklenemedi.' . ($uploadReason !== '' ? ' ' . $uploadReason : '');
                    break;
                }
                $uploadedGallery[] = $u;
            }
        ...
        if (empty($completionRow) && $newProofPhoto === null && empty($uploadedGallery)) {
            $errors[] = 'İlk tamamlanma onayı için en az bir fotoğraf kanıtı yükleyin.';
        }
        ...
        if (empty($errors)) {
            ...
            createNotification($listing['owner_id'], 'completion_requested', 'İş tamamlandı bildirimi ve kanıt gönderildi.', '/listings/detail.php?id=' . $id);
            setFlash('success', 'Tamamlanma kanıtı iletildi. Ev sahibinin onayı bekleniyor.');
            redirect(APP_URL . '/listings/detail.php?id=' . $id);
        }
    }
}
...
if ($isWorker && $_SERVER['REQUEST_METHOD'] === 'POST' && in_array(($_POST['action'] ?? ''), ['counter_accept', 'counter_reject'], true)) {
    if (!verifyCsrf()) {
        ...
        $myOffer = $chk->fetch();
        if (!$myOffer) {
            $errors[] = 'Karşı teklif bulunamadı.';
        } else {
            if ($a === 'counter_accept') {
                if ($workerHasScheduleConflict((int) $user['id'])) {
                    $errors[] = 'Bu saatte başka kabul edilmiş bir işin var. Karşı teklif kabul edilemedi.';
                } else {
                    ...
                    createNotification($listing['owner_id'], 'counter_accepted', 'Karşı teklifiniz kabul edildi.', '/listings/detail.php?id=' . $id);
                    setFlash('success', 'Karşı teklif kabul edildi.');
                    redirect(APP_URL . '/listings/detail.php?id=' . $id);
                }
            } else {
                ...
                $db->prepare("INSERT INTO offer_negotiations (offer_id, actor_id, event_type) VALUES (?, ?, 'counter_rejected')")
                    ->execute([$offerId, $user['id']]);
                createNotification($listing['owner_id'], 'counter_rejected', 'Karşı teklifiniz reddedildi.', '/listings/detail.php?id=' . $id);
                setFlash('success', 'Karşı teklif reddedildi.');
                redirect(APP_URL . '/listings/detail.php?id=' . $id);
            }
        }
    }
    ...
    if (!verifyCsrf()) {
        ...
        try {
            ...
            if ($hasAnyAvailability) {
                ...
                if (!$avail->fetch()) {
                    $errors[] = 'Takviminde bu tarih/saat müsait görünmüyor.';
                }
            }
        } catch (Exception $e) {}
        ...
        if (empty($errors)) {
            ...
            // Favoriye alan kullanicilara da ilan hareket bildirimi
            try {
                ...
                foreach ($favUsers->fetchAll(PDO::FETCH_COLUMN) as $favUserId) {
                    createNotification(
                        (int) $favUserId,
                        'favorite_update',
                        $listing['title'] . ' ilanına yeni teklif geldi.',
                        '/listings/detail.php?id=' . $id
                    );
                }
            } catch (Exception $e) {
                // Bildirim hatasi ana akisi kesmemeli.
            }
...
// Isi Tamamla ve Değerlendir (sadece ilan sahibi)
if ($isOwner && $listing['status'] === 'in_progress' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'complete_job') {
    if (!verifyCsrf()) {
        ...
        if ($rating < 1 || $rating > 5) $errors[] = 'Puan 1 ile 5 arasinda olmalidir.';
        
        if (empty($errors)) {
            if (empty($completionRow) || (int) ($completionRow['worker_confirmed'] ?? 0) !== 1) {
                $errors[] = 'İş kapatma için önce hizmet verenin kanıtlı tamamlanma onayı gerekli.';
            } elseif (($completionRow['completion_code'] ?? '') === '' || $completionCodeVerify !== (string) $completionRow['completion_code']) {
                $errors[] = 'Güvenli onay kodu eşleşmiyor.';
            }
        }
...
$shareUrl = APP_URL . '/listings/detail.php?id=' . $id;
?>
<!DOCTYPE html>
<html lang="tr">
...
<body>
                        ...
                        <div style="display:grid;grid-template-columns:1fr 360px;gap:24px;align-items:start;">
                                        ...
                                        <div
                                            ...
                                            <div>
                                                ...
                                                <div
                                                    ...
                                                    <span>
                                                        Saat: <?= $times[$listing['preferred_time']] ?? $listing['preferred_time'] ?>
                                                    </span>
                                                    <span>
                                            ...
                                            <div style="text-align:right;">
                                                <div class="listing-action-group">
                                                    <?php if ($isLoggedIn): ?>
                                                    <button id="favBtn" class="listing-icon-btn" onclick="toggleFavorite(<?= $id ?>)" title="Favorilere Ekle"
                                                        style="background:<?= $isFavorite ? 'rgba(239,68,68,0.1)' : 'var(--bg)' ?>;border-color:<?= $isFavorite ? '#ef4444' : 'var(--border)' ?>;">
                                                        <?= $isFavorite ? '&#10084;' : '&#9825;' ?>
                                                    </button>
                                                    <?php endif; ?>
                                                    <div class="listing-share-wrap">
                                                        <button id="shareBtn" class="listing-icon-btn" onclick="toggleShareMenu(event)" title="Paylaş"
                                                            aria-expanded="false" aria-controls="shareMenu">
                                                            &#8599;
                                                        </button>
                                                        <div id="shareMenu" class="listing-share-menu" aria-hidden="true">
                                                            <a href="https://wa.me/?text=<?= urlencode($listing['title'] . ' - ' . $shareUrl) ?>" target="_blank"
                                                                class="listing-share-link">
                                                                <span class="listing-share-link__icon">WA</span> WhatsApp
                                                            </a>
                                                            <a href="https://twitter.com/intent/tweet?text=<?= urlencode($listing['title'] . ' - Temizci Burada') ?>&url=<?= urlencode($shareUrl) ?>" target="_blank"
                                                                class="listing-share-link">
                                                                <span class="listing-share-link__icon">X</span> Twitter / X
                                                            </a>
                                                            <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($shareUrl) ?>" target="_blank"
                                                                class="listing-share-link">
                                                                <span class="listing-share-link__icon">FB</span> Facebook
                                                            </a>
                                                            <button type="button" onclick="copyLink()" class="listing-share-link listing-share-link--copy">
                                                                <span id="copyText">Linki Kopyala</span>
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div style="padding:18px 22px;border-bottom:1px solid var(--border-light);">
                                                ...
                                                <?php if (!empty($offer['counter_price']) && ($offer['counter_status'] ?? 'none') !== 'none'): ?>
                                                    <div style="margin:10px 0 0 56px;padding:10px 12px;background:var(--bg);border:1px solid var(--border);border-radius:10px;">
                                                        <div style="font-size:0.78rem;color:var(--text-muted);">Karşı Teklif</div>
                                                        ...
                                                        <?php foreach (array_slice($offerTimeline[(int) $offer['id']], 0, 4) as $tl): ?>
                                                            <?php
                                                                $eventLabel = match($tl['event_type']) {
                                                                    ...
                                                                    'counter_sent' => 'karşı teklif sundu',
                                                                    'counter_accepted' => 'karşı teklifi kabul etti',
                                                                    'counter_rejected' => 'karşı teklifi reddetti',
                                                ...
                                                <?php if ($isOwner && $offer['status'] === 'pending' && $listing['status'] === 'open'): ?>
                                                    <div style="display:flex;gap:8px;padding-left:56px;margin-top:10px;">
                                                        <a href="detail?id=<?= $id ?>&action=accept&offer_id=<?= $offer['id'] ?>"
                                                            ...
                                                            data-confirm="Bu teklifi kabul etmek istediginizden emin misiniz?"> Kabul Et</a>
                                                    ...
                                                    <form method="POST" style="display:flex;gap:8px;padding-left:56px;margin-top:8px;align-items:end;flex-wrap:wrap;">
                                                        ...
                                                        <div>
                                                            <label style="font-size:0.75rem;color:var(--text-muted);display:block;">Karşı Teklif (TL)</label>
                                                        ...
                                                        <button type="submit" class="btn btn-outline btn-sm">Karşı Teklif Gönder</button>
                                                    </form>
                                                <?php elseif ($isOwner): ?>
                                                    <div style="display:flex;gap:8px;padding-left:56px;margin-top:10px;">
                                                        <a href="../messages.php?uid=<?= $offer['user_id'] ?>" class="btn btn-outline btn-sm">Mesaj Gönder</a>
                                                ...
                                                <?php elseif ($isWorker && (int) $offer['user_id'] === (int) $user['id'] && ($offer['counter_status'] ?? 'none') === 'pending' && $listing['status'] === 'open'): ?>
                                                    <form method="POST" style="display:flex;gap:8px;padding-left:56px;margin-top:10px;">
                                                        <?= csrfField() ?>
                                                        <input type="hidden" name="offer_id" value="<?= (int) $offer['id'] ?>">
                                                        <button type="submit" name="action" value="counter_accept" class="btn btn-secondary btn-sm">Karşı Teklifi Kabul Et</button>
                            ...
                            <div>
                                ...
                                <?php elseif ($isOwner && $listing['status'] === 'in_progress'): ?>
                                    ...
                                    <div class="card">
                                        <div class="card-header">
                                            <div class="card-title"> Çift Onay ile İşi Tamamla</div>
                                        </div>
                                        <div class="card-body">
                                            <?php if (!empty($completionRow) && (!empty($completionPhotos) || !empty($completionRow['proof_photo']))): ?>
                                                <div style="margin-bottom:12px;">
                                                    <div style="font-size:0.8rem;color:var(--text-muted);margin-bottom:6px;">Hizmet Veren Kanıt Galerisi</div>
                                                    <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;">
                                                        <?php if (!empty($completionPhotos)): ?>
                                                            <?php foreach ($completionPhotos as $p): ?>
                                                                <img src="<?= UPLOAD_URL . e($p) ?>" alt="Kanıt" style="width:100%;border-radius:10px;border:1px solid var(--border);max-height:160px;object-fit:cover;">
                                                            <?php endforeach; ?>
                                                        <?php elseif (!empty($completionRow['proof_photo'])): ?>
                                                            <img src="<?= UPLOAD_URL . e($completionRow['proof_photo']) ?>" alt="Kanıt" style="width:100%;border-radius:10px;border:1px solid var(--border);max-height:220px;object-fit:cover;">
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
                                            ...
                                            <?php if (empty($completionRow) || (int) ($completionRow['worker_confirmed'] ?? 0) !== 1): ?>
                                                <div class="flash flash-info" style="margin-bottom:14px;">Hizmet verenin kanıtlı tamamlanma onayı bekleniyor.</div>
                                            <?php else: ?>
                                                <div class="flash flash-success" style="margin-bottom:14px;">Hizmet veren onay verdi. Siz onaylayınca iş kapanacak.</div>
                                            <?php endif; ?>
                                            <form method="POST">
                                                <?= csrfField() ?>
                                                <input type="hidden" name="action" value="complete_job">
                                                <div class="form-group">
                                                    <label class="form-label">6 Haneli Güvenli Onay Kodu</label>
                                                    <input type="text" name="completion_code_verify" class="form-control" maxlength="6" pattern="\d{6}" placeholder="Örn: 284731" required>
                                                    <div class="form-hint">Hizmet verenin girdiği kodu yazin.</div>
                                                </div>
                                ...
                                <?php elseif ($isWorker && $listing['status'] === 'in_progress' && (int) $user['id'] === $acceptedWorkerId): ?>
                                    <div class="card">
                                        <div class="card-header">
                                            <div class="card-title"> İş Tamamlama Kanıtı</div>
                                        </div>
                                        <div class="card-body">
                                            <?php if (!empty($completionRow) && (int) ($completionRow['worker_confirmed'] ?? 0) === 1): ?>
                                                <div class="flash flash-success" style="margin-bottom:12px;">Tamamlanma bildirimi gönderildi. Ev sahibi onayı bekleniyor.</div>
                                            <?php endif; ?>
                                            <form method="POST" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group">
                                                    <label class="form-label">6 Haneli Güvenli Onay Kodu</label>
                                                    <input type="text" name="completion_code" class="form-control" maxlength="6" pattern="\d{6}" value="<?= e($completionRow['completion_code'] ?? '') ?>" placeholder="Örn: 284731" required>
                                                    <div class="form-hint">Bu kodu ev sahibiyle paylaşın. İş kapanışında bu kod doğrulanır.</div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="form-label">Kısa Tamamlama Notu</label>
                                                    <textarea name="worker_note" class="form-control" rows="3" placeholder="Yaptığınız işle ilgili not düşün..."><?= e($completionRow['worker_note'] ?? '') ?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label class="form-label">Kanıt Fotoğrafı</label>
                                                    <input type="file" name="proof_photo" class="form-control" accept="image/*">
                                                    <div class="form-hint">İlk onayda en az bir fotoğraf beklenir.</div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="form-label">Kanıt Galerisi (Çoklu)</label>
                                                    <input type="file" name="proof_photos[]" class="form-control" accept="image/*" multiple>
                                                    <div class="form-hint">Birden fazla foto seçip toplu yükleyebilirsiniz.</div>
                                                </div>
                                                <?php if (!empty($completionPhotos) || !empty($completionRow['proof_photo'])): ?>
                                                    <div style="margin-bottom:12px;">
                                                        <div style="font-size:0.8rem;color:var(--text-muted);margin-bottom:6px;">Yüklü Kanıtlar</div>
                                                        <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;">
                                                            <?php if (!empty($completionPhotos)): ?>
                                                                <?php foreach ($completionPhotos as $p): ?>
                                                                    <img src="<?= UPLOAD_URL . e($p) ?>" alt="Mevcut Kanıt" style="width:100%;max-height:140px;object-fit:cover;border-radius:10px;border:1px solid var(--border);">
                                                                <?php endforeach; ?>
                                                            <?php elseif (!empty($completionRow['proof_photo'])): ?>
                                                                <img src="<?= UPLOAD_URL . e($completionRow['proof_photo']) ?>" alt="Mevcut Kanıt" style="width:100%;max-height:180px;object-fit:cover;border-radius:10px;border:1px solid var(--border);">
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                <?php endif; ?>
                                                <button type="submit" class="btn btn-primary btn-block">Tamamlandı Olarak Bildir</button>
                                            </form>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <form method="POST" data-validate>
                                                ...
                                                <div class="form-group">
                                                    <label class="form-label" for="price">Teklif Fiyatı (TL) *</label>
    ...
    <style>
        ...
        .listing-share-link + .listing-share-link {
            border-top: 1px solid var(--border-light);
        }

        .listing-share-link__icon {
            width: 24px;
            height: 24px;
            border-radius: 8px;
            border: 1px solid var(--border);
        ...
        html[data-theme="dark"] .listing-share-menu {
            background: #0f172a;
            border-color: rgba(148, 163, 184, 0.35);
            box-shadow: 0 18px 36px rgba(0, 0, 0, 0.45);
        }

        html[data-theme="dark"] .listing-share-link {
            color: #e2e8f0;
        }

        html[data-theme="dark"] .listing-share-link:hover {
            background: rgba(148, 163, 184, 0.12);
        }

        html[data-theme="dark"] .listing-share-link + .listing-share-link {
            border-top-color: rgba(148, 163, 184, 0.18);
        }

        html[data-theme="dark"] .listing-share-link__icon {
            background: rgba(148, 163, 184, 0.12);
            border-color: rgba(148, 163, 184, 0.35);
            color: #e2e8f0;
        }
        ...
        @media (max-width: 860px) {
            ...
            body.tb-has-mobile-nav .listing-share-menu {
                bottom: calc(var(--mobile-nav-h, 74px) + 16px + env(safe-area-inset-bottom));
            }
        }
    </style>
    <script>
    ...
    const listingShareUrl = <?= json_encode($shareUrl) ?>;
    const listingShareTitle = <?= json_encode($listing['title'] . ' - Temizci Burada') ?>;

    function setShareMenuState(open) {
    ...
    function toggleShareMenu(event) {
        ...
        if (isMobile && navigator.share && typeof navigator.share === 'function') {
            navigator.share({
                title: listingShareTitle,
                text: listingShareTitle,
                url: listingShareUrl
            }).catch(() => {});
            return;
        }
    ...
    async function copyLink() {
        ...
        const setCopyState = (text) => {
            ...
            setTimeout(() => {
                copyText.textContent = 'Linki Kopyala';
            }, 1800);
        };
        ...
        try {
            ...
            setCopyState('Kopyalandi');
            closeShareMenu();
        } catch (_) {
            setCopyState('Kopyalanamadi');
        }
    }
    </script>
</body>
