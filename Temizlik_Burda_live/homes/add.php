...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf()) {
        ...
        if (empty($title))
            $errors[] = 'Ev baÅŸlÄ±ÄŸÄ± zorunludur.';
        if (empty($address))
            $errors[] = 'Adres zorunludur.';
        if (empty($city))
            $errors[] = 'Åehir zorunludur.';
        if (empty($room))
            $errors[] = 'Oda yapÄ±sÄ± seÃ§iniz.';
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            if (!$photoPath) {
                ...
                $errors[] = 'FotoÄŸraf yÃ¼klenemedi.' . ($uploadReason !== '' ? ' ' . $uploadReason : '');
            }
        }
        ...
        if (empty($errors)) {
            ...
            setFlash('success', 'Ev baÅŸarÄ±yla eklendi!');
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <div class="container-sm">
                    ...
                    <div class="page-subtitle">Ev bilgilerinizi girin. Daha sonra ilanlarÄ±nÄ±zda bu evi
                    ...
                    <form method="POST" enctype="multipart/form-data" class="card" data-validate>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label class="form-label">Ev FotoÄŸrafÄ±</label>
                                <div class="photo-upload-area">
                                    <input type="file" id="photoInput" name="photo" accept="image/*">
                                    <div class="upload-icon"></div>
                                    <div class="upload-text">FotoÄŸraf yÃ¼klemek iÃ§in tÄ±klayÄ±n veya sÃ¼rÃ¼kleyin</div>
                                    <div class="upload-hint">JPG, PNG, WEBP, GIF veya HEIC Â· Maks. 12MB</div>
                                </div>
                                <img id="photoPreview" class="photo-preview" alt="Ã–nizleme">
                            </div>

                            <div class="grid-2" style="gap:16px;">
                                <div class="form-group" style="grid-column:span 2;">
                                    <label class="form-label" for="title">Ev BaÅŸlÄ±ÄŸÄ± / Lakap *</label>
                                    <input type="text" id="title" name="title" class="form-control"
                                        placeholder="Ã–rn: KadÄ±kÃ¶y Dairem, Annem'in Evi..."
                                        value="<?= e($_POST['title'] ?? '') ?>" required>
                                    <div class="form-hint">Bu isim sadece sizin iÃ§in gÃ¶rÃ¼necek, ilanlarÄ±nÄ±zda seÃ§im
                                        yaparken kullanÄ±lacak.</div>
                                </div>
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="district">Ä°lÃ§e</label>
                                    <input type="text" id="district" name="district" class="form-control"
                                        placeholder="KadÄ±kÃ¶y" value="<?= e($_POST['district'] ?? '') ?>">
                                </div>
                        ...
                        <div class="card-header" style="border-top:1px solid var(--border);">
                            <div class="card-title"> Ev Ã–zellikleri</div>
                        </div>
                        <div class="card-body">
                            <div class="grid-2" style="gap:16px;">

                                <div class="form-group">
                                    <label class="form-label" for="room_config">Oda YapÄ±sÄ± *</label>
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="bathroom_count">Banyo SayÄ±sÄ±</label>
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="sqm">Metrekare (mÂ²)</label>
                                    <input type="number" id="sqm" name="sqm" class="form-control" placeholder="120"
                                        min="10" max="1000" value="<?= e($_POST['sqm'] ?? '') ?>">
                                </div>

                                <div class="form-group" style="grid-column:span 2;">
                                    <label class="form-label">AsansÃ¶r</label>
                                    <div class="toggle-wrapper">
                                        <label class="toggle">
                                            <input type="checkbox" name="has_elevator" <?= isset($_POST['has_elevator']) ? 'checked' : '' ?>>
                                            <span class="toggle-slider"></span>
                                        </label>
                                        <span style="color:var(--text-secondary);font-size:0.88rem;">Binada asansÃ¶r
                                            var</span>
                                    </div>
                                </div>

                                <div class="form-group" style="grid-column:span 2;">
                                    <label class="form-label" for="notes">Ã–zel Notlar</label>
                                    <textarea id="notes" name="notes" class="form-control" rows="3"
                                        placeholder="Temizlik iÃ§in bilmesi gereken Ã¶zel durumlar... (Ã¶r: kÃ¶pek var, hassas parke, vb.)"
                        ...
                        <div class="card-footer" style="display:flex;gap:12px;justify-content:flex-end;">
                            <a href="list" class="btn btn-ghost">Ä°ptal</a>
