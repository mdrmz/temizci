    ...
    if ($action === 'logout_other_sessions') {
        ...
        if (!verifyCsrf()) {
            ...
            if (isset($_FILES['avatar']) && ($_FILES['avatar']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
                ...
                if ($uploaded === false) {
                    ...
                    $errors[] = 'Profil fotoÄŸrafÄ± yÃ¼klenemedi.' . ($uploadReason !== '' ? ' ' . $uploadReason : '');
                } else {
                    $newAvatar = $uploaded;
                    $avatarChanged = true;
                }
            ...
            if (empty($errors)) {
                ...
                if ($stmt->fetch()) {
                    ...
                    if ($hasCurrent && $hasNew) {
                        $passwordError = tbValidatePasswordPolicy($newPassword, (string) ($user['email'] ?? ''), (string) ($user['name'] ?? ''));
                        if ($passwordError !== null) {
                            $errors[] = $passwordError;
                        } elseif ($currentPassword === $newPassword) {
                            $errors[] = 'Yeni sifre mevcut sifreyle ayni olamaz.';
        ...
        if (!verifyCsrf()) {
            ...
            if ($pkgTitle === '' || $pkgPrice <= 0) {
                $errors[] = 'Paket baÅŸlÄ±ÄŸÄ± ve geÃ§erli fiyat zorunludur.';
            } else {
...
$trustLevel = $trustScore >= 80 ? 'YÃ¼ksek GÃ¼ven' : ($trustScore >= 50 ? 'Orta GÃ¼ven' : 'GeliÅŸimde');
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <div class="profile-shell">
                    ...
                    <div class="card mt-4">
                        <div class="card-header"><div class="card-title">GÃ¼ven Profili</div></div>
                        <div class="card-body">
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:12px;">
                                ...
                                <span class="badge <?= $trustChecks['city'] ? 'badge-open' : 'badge-closed' ?>">Åehir</span>
                                <span class="badge <?= $trustChecks['verified'] ? 'badge-open' : 'badge-closed' ?>">DoÄŸrulama</span>
                        ...
                        <div class="card-body">
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="avatarInput">Profil FotoÄŸrafÄ±</label>
                                    <div style="display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap;">
                                        <div class="avatar avatar-xl" style="overflow:hidden;">
                                            <img id="avatarPreview" src="<?= $avatarUrl ?>" alt="Profil FotoÄŸrafÄ±"
                                                style="width:100%;height:100%;object-fit:cover;border-radius:50%;<?= $avatarUrl ? 'display:block;' : 'display:none;' ?>">
                                            <span id="avatarInitials" style="<?= $avatarUrl ? 'display:none;' : '' ?>"><?= e($initials) ?></span>
                                        </div>
                                        <div style="flex:1;min-width:240px;">
                                            <div id="avatarUploadArea" class="photo-upload-area" style="padding:18px;">
                                                <input type="file" id="avatarInput" name="avatar" accept="image/*">
                                                <div class="upload-icon"></div>
                                                <div class="upload-text">Profil fotoÄŸrafÄ± yÃ¼klemek iÃ§in tÄ±klayÄ±n veya sÃ¼rÃ¼kleyin</div>
                                                <div class="upload-hint">JPG, PNG, WEBP, GIF veya HEIC Â· Maks. 12MB</div>
                                            </div>
                                            <div class="form-hint">FotoÄŸraf seÃ§ince Ã¶nizleme otomatik gÃ¼ncellenir.</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="telegram_chat_id" name="telegram_chat_id" class="form-control" value="<?= e($user['telegram_chat_id'] ?? '') ?>" placeholder="Ã–rn: 123456789">
                            ...
                            <div class="card-body">
                                <form method="POST" class="mb-3">
                                    ...
                                    <div class="grid-2" style="gap:10px;">
                                        <div class="form-group">
                                            <label class="form-label">Paket BaÅŸlÄ±ÄŸÄ±</label>
                                            <input type="text" name="pkg_title" class="form-control" placeholder="Ã–rn: 2+1 DetaylÄ± Temizlik" required>
                                        </div>
                                        <div class="form-group">
                                            <label class="form-label">BaÅŸlangÄ±Ã§ FiyatÄ± (TL)</label>
                                            <input type="number" name="pkg_price" class="form-control" min="1" step="1" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label">AÃ§Ä±klama</label>
                                        <textarea name="pkg_desc" class="form-control" rows="2" placeholder="Paketin iÃ§eriÄŸi..."></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-primary btn-sm">Paket Ekle</button>
                                </form>
                                <?php if (empty($servicePackages)): ?>
                                    <div style="font-size:0.85rem;color:var(--text-muted);">HenÃ¼z paket tanÄ±mlamadÄ±nÄ±z.</div>
                                <?php else: ?>
